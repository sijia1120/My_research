...
import pandas as pf
merged_latent = pd.read_pickle("/kaggle/input/year2-phd-dataset/merged_latent49.pkl")
...
class ResidualBlockReduce(tf.keras.layers.Layer):
    def __init__(self, n_layers, n_neurons, **kwargs):
        super().__init__(**kwargs)
        self.hidden = [tf.keras.layers.Dense(n_neurons // (2 ** i), activation="relu",
                                             kernel_initializer="he_normal")
...
class ResidualBlockFinal(tf.keras.layers.Layer):
    def __init__(self, n_layers, n_neurons, **kwargs):
        super().__init__(**kwargs)
        self.hidden = [tf.keras.layers.Dense(n_neurons, activation = "relu",
                                         kernel_initializer = "he_normal")
...
class ReconstructingRegressor(keras.models.Model):
    def __init__(self, output_dim, **kwargs):
        ...
        self.block1 = ResidualBlockFinal(n_layers=2, n_neurons = 49) # originally 49
        self.block2 = ResidualBlockFinal(n_layers=2, n_neurons = 49)
    ...
    @staticmethod
    def reconstruction_loss(reconstruction, inputs, rate=0.01): # 0.05
...
for random_seed in seed_list:
    ...
    model.compile(optimizer="nadam", loss="mse", metrics=[my_metric_fn])
